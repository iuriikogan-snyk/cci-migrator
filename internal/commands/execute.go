...
func (c *ExecuteCommand) Execute() error {
	...
	go func() {
		...
		for i, policy := range policies {
			...
			// If the policy has no asset key, skip it. Missing asset keys mean we
			// couldn't match the original finding to an asset; per new fault
			// tolerance rules, skip these and continue with the rest.
			if strings.TrimSpace(policy.AssetKey) == "" {
				log.Printf("Skipping policy %d/%d (internal_id=%s): missing asset key - associated ignores will be skipped", i+1, totalPolicies, policy.InternalID)
				skippedPolicies++
				continue
			}
			...
			if externalID == "" {
				// If API indicated the policy already exists (409) but returned no ID,
				// create a stable placeholder. Avoid using an empty asset key in the
				// placeholder; use the internal ID as fallback (though we should
				// rarely get here because policies without asset keys are skipped).
				if strings.TrimSpace(policy.AssetKey) == "" {
					log.Printf("Policy for internal id %s likely already exists (409 conflict), using internal-id based placeholder", policy.InternalID)
					externalID = fmt.Sprintf("existing-policy-internal-%s", policy.InternalID)
				} else {
		...
		log.Printf("  Policies skipped (missing asset key): %d", skippedPolicies)
