...
func (c *VerifyCommand) Execute() error {
	...
	// Note: Missing asset keys (mapping from findings -> asset_key) are tolerated.
	// Ignores that cannot be matched to an asset key will be skipped during migration
	// but should not cause the overall verification to be considered INCOMPLETE.
	if missingTargetInfo > 0 || metadataCount == 0 {
	...
	if missingAssetKeys > 0 {
		fmt.Printf("NOTE: %d ignores are missing asset keys and will be skipped during migration.\n", missingAssetKeys)
	}
